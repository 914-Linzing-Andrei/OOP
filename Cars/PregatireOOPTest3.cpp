...
void PregatireOOPTest3::showCarsWithSpecificManufacturer() {
    ...
    int numberOfCarsWithGivenManufacturer = count_if(allCars.begin(), allCars.end(), [&](Car car) {
        return car.getManufacturer() == manufacturer;
        }
    );
    string number = to_string(numberOfCarsWithGivenManufacturer);
    if(number != "0")
        this->ui.carsWithSpecificManufacturerlistWidget->addItem(QString::fromStdString(number));
